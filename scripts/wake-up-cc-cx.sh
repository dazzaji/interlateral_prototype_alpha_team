#!/bin/bash
# wake-up-cc-cx.sh - Canonical entrypoint for CC with observability (CC + Codex ONLY)
#
# Usage:
#   ./scripts/wake-up-cc-cx.sh
#   ./scripts/wake-up-cc-cx.sh "Your prompt here"
#   ./scripts/wake-up-cc-cx.sh --dangerously-skip-permissions "Your prompt here"
#   WAKEUP_PROMPT_FILE=/tmp/prompt.txt ./scripts/wake-up-cc-cx.sh --dangerously-skip-permissions
#
# Notes:
# - If the flag is omitted, this script defaults to --dangerously-skip-permissions.
# - If no prompt is provided, a safe default wake-up prompt is used that only
#   requests ACK from Codex and has CC + Codex ask: "What is our assignment?"
...
# If no prompt was provided, inject a default "CC + Codex only" wake-up prompt.
# Heuristic: if all args are flags (start with -), treat prompt as missing.
HAS_PROMPT=0
for arg in "$@"; do
    if [[ "$arg" != -* ]]; then
        HAS_PROMPT=1
...
if [[ "$HAS_PROMPT" -eq 0 ]]; then
    set -- "$@" "Read CLAUDE.md. Execute the Wake-Up Protocol, but ONLY seek ACK from Codex (no AG, no Gemini). Once both ACKed, BOTH agents ask: What is our assignment?"
    echo "[wake-up] Using default CC+Codex-only wake-up prompt"
fi
...
# STEP 1: Bootstrap the system BEFORE CC wakes
echo "Step 1: System bootstrap (CC + Codex only)..."
if [ -x "$REPO_ROOT/scripts/bootstrap-full-cc-cx.sh" ]; then
    "$REPO_ROOT/scripts/bootstrap-full-cc-cx.sh" || {
        echo ""
    ...
    echo "WARNING: bootstrap-full-cc-cx.sh not found. Skipping system bootstrap."
fi
